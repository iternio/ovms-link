...
describe('calculateMaxElapsedDuration cadence selector', () => {
  ...
  // KNOWN QUIRK (pending fix-vs-accept decision): on SUBSOL/TOYBZ4X during AC
  // charge the vehicle module drops v.e.gear, so is_parked is ABSENT (undefined).
  // The `!is_parked` row (branch 3) matches first, throttling AC charging to 160 s
  // instead of the intended 30 min (branch 4). This documents the CURRENT behavior;
  // if the cadence is fixed to treat is_charging as parked, flip this expectation.
  test('known SUBSOL quirk: AC charging with is_parked ABSENT falls into the 160 s path', () => {
    const abrp = loadAbrp()
...
describe('lifecycle / event flow', () => {
  ...
  test('manageVehicleStateEvents(true) wires the vehicle/ticker topics + sets active; (false) unsubscribes the on/off events', () => {
    ...
    // vehicle.type.set and ticker.10 are intentionally NOT unsubscribed on stop —
    // assert they persist so an unintended teardown change is caught.
    expect(after).toContain('vehicle.type.set')
    expect(after).toContain('ticker.10')
