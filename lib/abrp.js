...
function median(array) {
  ...
  if (sorted.length % 2 === 0) {
    // even
    return average(sorted.slice(midpoint, midpoint + 1))
  } else {
    // odd
...
function sendTelemetryIfNecessary() {
  ...
  // using the average from the metrics collected at a higher frequency
  const smoothedMetrics = getAndClearSmoothedMetrics()
  if (!isNil(smoothedMetrics.averagePower)) {
    currentTelemetry.power = smoothedMetrics.averagePower
  }
  if (!isNil(smoothedMetrics.averageSpeed)) {
    currentTelemetry.speed = smoothedMetrics.averageSpeed
  }
